...
import { getUserProgress, saveUserProgress } from '../services/localDatabaseService';
import { addToSyncQueue } from '../services/localDatabaseService';

export function useLocalUserProgress(mode?: string) {
  ...
  const loadProgress = useCallback(async () => {
    ...
    try {
      const data = await getUserProgress(user.id, mode);
      setProgress(data);
    } catch (err) {
  ...
  const updateProgress = useCallback(async (
    ...
    try {
      const progressId = await saveUserProgress(
        user.id,
        mode,
        updates
      );
